    ...
    <div class="info-page">
      ...
      <div class="info-wrapper">
        ...
        <div class="info-list">
          ...
          <a-input v-model="detail.room"></a-input>
        </div>
      </div>
    </div>
...
<script>
import {selectOld, updateOld} from "@/services/oldPerson";
...
export default {
  ...
  methods: {
    ...
    loadImg(){
      const _this = this
      selectOld(_this.detail.id).then(function (resp){
    ...
    update(){
      const _this = this
      formData.append('old', JSON.stringify(_this.detail)); // 将老人信息转换为JSON字符串并添加到FormData中
      updateOld(formData).then(function (resp){
        //console.log(resp.data.code)
        if(resp.data.code === 200){
          console.log(200)
          alert(resp.data.msg)
          _this.$router.push("/list/query");
        }else {
          alert(resp.data.msg)
        }
      })
